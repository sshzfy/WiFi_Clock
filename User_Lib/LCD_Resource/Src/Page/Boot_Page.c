...
{
    ST7789_Draw_Picture(0, 0, (const Image_t *)&Boot_Page_Waitconnect); // 绘制开机页�? 等待连接
    ...
    if (!wifi_ok)
    {
        ...
        ST7789_Fill_Color(0, 170, WIDTH - 1, HEIGHT - 1, COLOR_WHITE);                     // 填充白色背景,清除旧信�?
    ...
    else
    {
        ...
        int x1 = (WIDTH - w1) / 2; // �?行的起始X
        int x2 = (WIDTH - w2) / 2; // �?行的起始X
        int x3 = (WIDTH - w3) / 2; // �?行的起始X
        ...
        ST7789_Fill_Color(0, 200, WIDTH - 1, HEIGHT - 1, COLOR_WHITE);                     // 填充白色背景,清除旧信�?
